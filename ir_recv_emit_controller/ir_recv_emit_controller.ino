...
// const uint8_t BUZZER_PIN = 13; // ブザーピン
...
void loop() {
  ...
  */
  ...
  if (Serial.available()) {
    ...
    if (input.startsWith("SEND_IR ")) {
      String jsonStr = input.substring(8);
      IRData irDataSend;
...
void sendIrData(IRData irDataSend) {
  ...
  case SONY:
    ...
    break;
  case RC5:
    IrSender.sendRC5(irDataSend.address, irDataSend.command,
                     IR_SEND_REPEAT_NUM);
    break;

  case RC6:
    IrSender.sendRC6(irDataSend.address, irDataSend.command,
                     IR_SEND_REPEAT_NUM);
    break;

  case PANASONIC:
    IrSender.sendPanasonic(
      (uint16_t)irDataSend.address,
      (uint8_t)irDataSend.command,
      IR_SEND_REPEAT_NUM);
    break;

  case JVC:
    IrSender.sendJVC(
      (uint8_t)irDataSend.address,
      (uint8_t)irDataSend.command,
      IR_SEND_REPEAT_NUM);
    break;

  case SAMSUNG:
    IrSender.sendSamsung(irDataSend.address, irDataSend.command,
                         IR_SEND_REPEAT_NUM);
    break;

  case LG:
    IrSender.sendLG(irDataSend.address, irDataSend.command, IR_SEND_REPEAT_NUM);
    break;

  case DENON:
    IrSender.sendDenon(irDataSend.address, irDataSend.command,
                       IR_SEND_REPEAT_NUM);
    break;

  case SHARP:
    IrSender.sendSharp(irDataSend.address, irDataSend.command);
    break;

  // 他のプロトコルも必要に応じて追加
  default:
    // Serial.println("Unsupported protocol for direct send.");
    Serial.println("UNKNOWN protocol → fallback NEC");
    IrSender.sendNEC(irDataSend.address, irDataSend.command,
                     IR_SEND_REPEAT_NUM);
  }
  delay(20);
  // buzzer
  // tone(BUZZER_PIN, NOTE_C5, BUZZER_DURATION);
  return;
}
